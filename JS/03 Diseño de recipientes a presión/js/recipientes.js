function calcularepFcn() {
    ...
    Vtapas = Math.PI/12*(Math.pow(diametro + 2*espesortapareal,3) - Math.pow(diametro,3));
        ...
        // Cálculo del peso del equipo (solo estructura)
        ...
        volumenliquido = Math.PI * Math.pow(diametro / 2, 2) * nivel;
        document.querySelector('#volumenliquido').value = volumenliquido.toFixed(2);

        // Cálculo del peso del líquido
        pesoliquido = volumenliquido * rho;
        document.querySelector('#pesoliquido').value = pesoliquido.toFixed(0);

        // Suma del peso del líquido al peso del equipo lleno
